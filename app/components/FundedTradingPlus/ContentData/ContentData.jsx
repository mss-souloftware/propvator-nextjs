...
export default function ContentData() {
  ...
  const filteredData = tableBodyData.filter((item) => {
    const matchesFirm = item.firm === "Funded Trading Plus";
    const matchesStep =
      ...
      <section className="comparison--table--area">
        ...
        <div className={`${filterStyle.filterButtons} mb-4`}>
          ...
          <button
            onClick={() =>
              setSelectedStep((prevStep) =>
                prevStep.includes("2 Step")
                  ? prevStep.filter((step) => step !== "2 Step")
                  : [...prevStep, "2 Step"]
              )
            }
            className={`${filterStyle.filterStepButton} ${
              selectedStep.includes("2 Step") ? "bg-[#3338D3]" : ""
            }`}
          >
            2 Step
          </button>
          ...
          <button
            onClick={() =>
              setSelectedStep((prevStep) =>
                prevStep.includes("3 Step")
                  ? prevStep.filter((step) => step !== "3 Step")
                  : [...prevStep, "3 Step"]
              )
            }
            className={`${filterStyle.filterStepButton} ${
              selectedStep.includes("3 Step") ? "bg-[#3338D3]" : ""
            }`}
          >
            3 Step
          </button>
              ...
              <table className="w-100" id="data-table">
                ...
                <tbody id="table-body">
                  {currentListings.length > 0 ? (
                    currentListings
                      .filter((item) => item.firm === "Funded Trading Plus")
                      .map((item, index) => (
